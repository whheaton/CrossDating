 ...
 *
 */
public class SeriesPanel extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener{

	/**
	 * 
	 */
	private static final long serialVersionUID = -434475099544065140L;
	private RingSeries the_series;
	private int the_max;
	...
	public SeriesPanel(RingSeries series){
		super();
		the_series = series;
		x_offset = 0;
		...
		the_max = 3200;
		ring_mean = (double)the_max/(double)the_series.numRings();
	...
	protected void paintComponent(Graphics g){
		...
		int last = 0;
		for(int ii = 0; ii < the_series.numRings(); ii++){
			//System.out.println("ring "+ii+" "+the_master.getRing(ii));
			if(the_series.getRing(ii) < -max_sd){
				g2.setColor(Color.red);
				g2.drawLine(last+1+x_offset, y1, last+1+x_offset, y2);
				last++;
				g2.setColor(oldcolor);
			}
			else if(the_series.getRing(ii) > max_sd){
				g2.setColor(Color.red);
				...
				int x1 = (int)(last + ring_mean + ring_sd*the_series.getRing(ii));
